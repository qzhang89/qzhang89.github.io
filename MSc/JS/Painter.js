...
function Painter(canvas) {
    ...
    this.xOrigin       = this.marginLeft-this.xMin*this.xScale;
    this.yOrigin       = this.marginTop+this.plotHeight+this.yMin*this.yScale;
    this.zoomXOrigin   = 0;
    this.zoomYOrigin   = 0;
    this.xLabel        = "";
    this.yLabel        = "";
        ...
        canvas.addEventListener("keydown", function(evt) {
            if(painter.control) {
                ...
                if (evt.keyCode==87||evt.keyCode==83||evt.keyCode==65||evt.keyCode==68) {
                    ...
                    painter.setXRange(painter.xMin+(painter.xMin-painter.zoomXOrigin)*0.05*zoomX,
                                      painter.xMax+(painter.xMax-painter.zoomXOrigin)*0.05*zoomX);
                    painter.setYRange(painter.yMin+(painter.yMin-painter.zoomYOrigin)*0.05*zoomY,
                                      painter.yMax+(painter.yMax-painter.zoomYOrigin)*0.05*zoomY);
 ...
 */
Painter.prototype.setMargin = function(marginLeft, marginRight, marginTop, marginBottom) {
...
Painter.prototype.setXRange = function(xMin, xMax) {
    ...
    this.xOrigin = this.marginLeft-xMin*this.xScale;
    if (!this.refreshFlag) {
...
Painter.prototype.setYRange = function(yMin, yMax) {
    ...
    this.yOrigin = this.marginTop+this.plotHeight+yMin*this.yScale;
    if (!this.refreshFlag) {
...
Painter.prototype.setScale = function(xScale, yScale) {
    this.xScale = xScale;
    this.yScale = yScale;
    this.xMin = (this.marginLeft-this.xOrigin)/this.xScale;
    this.xMax = (this.marginLeft+this.plotWidth-this.xOrigin)/this.xScale;
    this.yMin = (this.yOrigin-this.marginTop-this.plotHeight)/this.yScale;
    this.yMax = (this.yOrigin-this.marginTop)/this.yScale;
};
...
Painter.prototype.setZoomOrigin = function(zoomXOrigin, zoomYOrigin) {
    this.zoomXOrigin = zoomXOrigin;
    this.zoomYOrigin = zoomYOrigin;
};
...
Painter.prototype.setMarkerSize = function(markerSize) {
    this.markerSize = markerSize;
};

Painter.prototype.setMarkerType = function(markerType) {
    this.markerType = markerType;
};

Painter.prototype.setLineOn = function(lineOn) {
    this.lineOn = lineOn;
};

Painter.prototype.setMarkerOn = function(markerOn) {
...
Painter.prototype.setControl = function(control) {
    ...
    if (control) {
        this.canvas.title = "1. Click to focus on the plot\n" +
                            ...
                            "3. Press \"Z\" and drag to zoom a region\n" +
...
Painter.prototype.plot = function(x, y, color) {
    ...
    cntx.beginPath();
    cntx.rect(this.marginLeft, this.marginTop, this.plotWidth, this.plotHeight);
    cntx.clip();
    ...
    if (this.markerOn) {
        for (var i=0; i<n; i++) {
            ...
            pointX = Math.round(this.xScale*x[i]);
            pointY = Math.round(this.yScale*y[i]);
            cntx.arc(this.xOrigin+pointX,this.yOrigin-pointY,this.markerSize,0,2*Math.PI);
...
Painter.prototype.plotEnergy = function(E, text, color) {
    ...
    // Draw energy
    cntx.strokeStyle = color;
    if (Ey>marginTop&&Ey<marginTop+plotHeight) {
        cntx.beginPath();
        cntx.moveTo(marginLeft, Ey);
        cntx.lineTo(marginLeft+plotWidth, Ey);
        cntx.stroke();
    }
    ...
    cntx.fillStyle = color;
    if (Ey-11>marginTop&&Ey-5<marginTop+plotHeight) {
        cntx.fillText(text+"    "+"E = "+E.toFixed(3), marginLeft+plotWidth-10, Ey-5);
    }
};
...
Painter.prototype.drawAxis = function() {
    ...
    cntx.fillRect(marginLeft, marginTop, plotWidth, plotHeight);
    ...
    var xOdr = xLog-Math.floor(xLog)<Math.log(3.2)/Math.log(10) ? Math.floor(xLog)-1 : Math.floor(xLog);
    var yOdr = yLog-Math.floor(yLog)<Math.log(3.2)/Math.log(10) ? Math.floor(yLog)-1 : Math.floor(yLog);
    var xFix = xOdr>=0 ? 0 : Math.abs(xOdr);
    var yFix = yOdr>=0 ? 0 : Math.abs(yOdr);
    var xGrid = Math.pow(10, xOdr);
    if (plotWidth/xScale/Math.pow(10, xOdr)<7) {
        xGrid = Math.pow(10, xOdr);
    } else if (plotWidth/xScale/Math.pow(10, xOdr)<14) {
        xGrid = 2*Math.pow(10, xOdr);
    } else {
        xGrid = 5*Math.pow(10, xOdr);
    }
    var yGrid = Math.pow(10, yOdr);
    if (plotHeight/yScale/Math.pow(10, yOdr)<7) {
        yGrid = Math.pow(10, yOdr);
    } else if (plotHeight/yScale/Math.pow(10, yOdr)<14) {
        yGrid = 2*Math.pow(10, yOdr);
    } else {
        yGrid = 5*Math.pow(10, yOdr);
    }
    ...
    var xTickLabelHeight = marginTop+plotHeight+this.tickLabelSize+2;
    var yTickLabelWidth  = marginLeft-4;
    for (var i=0; i<10000; i++) {
        xTickP = xOrigin+xGrid*i*xScale;
        xTickN = xOrigin-xGrid*i*xScale;
        yTickP = yOrigin-yGrid*i*yScale;
        yTickN = yOrigin+yGrid*i*yScale;
        if (xTickP>=marginLeft&&xTickP<=marginLeft+plotWidth) {
            cntx.textAlign = "center";
            cntx.moveTo(xTickP, marginTop+plotHeight);
            cntx.lineTo(xTickP, marginTop+plotHeight-tickSize);
            if (Math.abs(xOdr)<4) {
        ...
        if (i!=0&&xTickN>=marginLeft&&xTickN<=marginLeft+plotWidth) {
            cntx.textAlign = "center";
            cntx.moveTo(xTickN, marginTop+plotHeight);
            cntx.lineTo(xTickN, marginTop+plotHeight-tickSize);
            if (Math.abs(xOdr)<4) {
        ...
        if (yTickP>=marginTop&&yTickP<=marginTop+plotHeight) {
            cntx.textAlign = "right";
            cntx.moveTo(marginLeft, yTickP);
            cntx.lineTo(marginLeft+tickSize, yTickP);
            if (Math.abs(yOdr)<4) {
        ...
        if (i!=0&&yTickN>=marginTop&&yTickN<=marginTop+plotHeight) {
            cntx.textAlign = "right";
            cntx.moveTo(marginLeft, yTickN);
            cntx.lineTo(marginLeft+tickSize, yTickN);
            if (Math.abs(yOdr)<4) {
        ...
        if (xTickP>marginLeft+plotWidth&&xTickN<marginLeft&&
            yTickP<marginTop&&yTickN>marginTop+plotHeight) {
            break;
        }
    }
    cntx.stroke();
    ...
    cntx.textAlign = "center";
    cntx.fillText(this.xLabel, marginLeft+plotWidth*0.5, canvasHeight-1);
    cntx.rotate(-Math.PI/2);
    cntx.fillText(this.yLabel, -marginTop-plotHeight*0.5, this.labelSize);
    cntx.rotate(Math.PI/2);
